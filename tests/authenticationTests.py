...
def tokenTest ( response, user, tokenField, secret, expectedType, expectedSubject, expectedForename, expectedSurname, rolesField, expectedRole, expectedExpiresDelta ):
    assert tokenField in response, f"Login response error, {tokenField} field missing for warehouse {user}.";

    token = decode ( response[tokenField], key = secret, algorithms = ["HS256"] );

    assert "nbf"      in token, f"{tokenField} error for warehouse {user}, field nbf is missing."
    assert "type"     in token, f"{tokenField} error for warehouse {user}, field type is missing."
    assert "exp"      in token, f"{tokenField} error for warehouse {user}, field exp is missing."
    assert "sub"      in token, f"{tokenField} error for warehouse {user}, field sub is missing."
    assert "forename" in token, f"{tokenField} error for warehouse {user}, field forename is missing."
    assert "surname"  in token, f"{tokenField} error for warehouse {user}, field surname is missing."
    assert rolesField in token, f"{tokenField} error for warehouse {user}, field {rolesField} is missing."
    ...
    assert type     == expectedType                            , f"{tokenField} error for warehouse {user}, field type has an incorrect value, expected {expectedType}, got {type}."
    assert sub      == expectedSubject                         , f"{tokenField} error for warehouse {user}, field sub has an incorrect value, expected {expectedSubject}, got {sub}."
    assert forename == expectedForename                        , f"{tokenField} error for warehouse {user}, field forename has an incorrect value, expected {expectedForename}, got {forename}."
    assert surname  == expectedSurname                         , f"{tokenField} error for warehouse {user}, field surname has an incorrect value, expected {expectedSurname}, got {surname}."
    assert (roles   == expectedRole) or (expectedRole in roles), f"{tokenField} error for warehouse {user}, field {rolesField} has an incorrect value, expected {expectedRole}, got {roles}."

    expiresDelta = datetime.fromtimestamp ( exp ) - datetime.fromtimestamp ( nbf );

    assert expiresDelta.total_seconds ( ) == expectedExpiresDelta, f"{tokenField} error for warehouse {user}, expiration has an incorrect value, expected {expectedExpiresDelta}, got {expiresDelta.total_seconds ( )}."
...
def runAuthenticationTests ( authenticationAddress, jwtSecret, rolesField, customerRole, managerRole, administratorRole ):
    tests = [
        ...
        ["post", authenticationAddress + "/delete", setUpAdminHeaders ( True, authenticationAddress )      , { }, { "email": "john@gmail.a"   }, { }, 400, { "message": "Invalid email."           }, equals, 1],
        ["post", authenticationAddress + "/delete", setUpAdminHeaders ( True, authenticationAddress )      , { }, { "email": "john@gmail.com" }, { }, 400, { "message": "Unknown warehouse."            }, equals, 1],
